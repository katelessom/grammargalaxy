...
function Shop({ profile, onBuy, onBuyHint }: { profile: Profile; onBuy: (item: (typeof shopItems)[number]) => void; onBuyHint: () => void }) {
  return <section className="section shop-page"><div className="shop-hero"><img src="./assets/scenes/shop.webp" alt="A futuristic orbital rewards boutique" /><div><span className="eyebrow">ORBITAL MARKET</span><h2>Space Shop</h2><p>Spend mission Stardust on illustrated cabin objects or recharge limited mission boosts.</p><span className="shop-balance"><Icon name="dust" />{profile.stardust} Stardust</span></div></div><div className="boost-panel"><div className="boost-copy"><span className="eyebrow">MISSION BOOST</span><h3>Hint Charge</h3><p>Use hints inside grammar tasks. This boost is not a cabin decoration and will not appear in your room.</p><small>{profile.hints} / 9 charges available</small></div><div className="boost-art"><img src="./assets/ui/hint-charge.png" alt="Holographic hint charge token" /></div><button className="primary-button small" onClick={onBuyHint} disabled={profile.stardust < 35 || profile.hints >= 9}><Icon name="dust" />Buy hint 35</button></div><div className="shop-catalog-title"><div><span className="eyebrow">CABIN COLLECTION</span><h3>Twenty objects to collect</h3></div><span>{profile.purchases.length} owned / {profile.equippedItems.length} displayed</span></div><div className="shop-grid">{shopItems.map((item) => { const owned = profile.purchases.includes(item.id); const displayed = profile.equippedItems.includes(item.id); return <article className={`shop-card ${owned ? "owned" : ""} ${displayed ? "equipped" : ""}`} key={item.id}><img src={item.image} alt={item.name} /><h3>{item.name}</h3><p>{displayed ? "Placed in your cabin" : owned ? "Owned. Tap to place it in the cabin." : item.kind}</p><button className="primary-button small" onClick={() => onBuy(item)} disabled={!owned && profile.stardust < item.price}>{displayed ? "Remove from cabin" : owned ? "Place in cabin" : <><Icon name="dust" />Buy {item.price}</>}</button></article>; })}</div></section>;
}
